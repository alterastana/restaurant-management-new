...
@section('header')
	<h1 class="text-xl font-semibold text-gray-800">Tambah Customer</h1>
@endsection
		...
		<form action="{{ route('Dashboard.customer.store') }}" method="POST">
			...
			<div class="grid grid-cols-1 gap-4">
				...
				<label>
					...
					<input type="email" name="email" value="{{ old('email') }}" class="w-full mt-1 p-2 border rounded">
				</label>

				<label>
					<div class="text-sm font-medium text-gray-700">Telepon</div>
					<input type="text" name="phone" value="{{ old('phone') }}" class="w-full mt-1 p-2 border rounded">
				</label>

				<label>
					<div class="text-sm font-medium text-gray-700">Alamat</div>
					<textarea name="address" class="w-full mt-1 p-2 border rounded" rows="4">{{ old('address') }}</textarea>
				</label>

				<div class="flex gap-2 justify-end">
					<a href="{{ route('Dashboard.customer.index') }}" class="px-4 py-2 rounded bg-gray-200">Batal</a>
					<button type="submit" class="px-4 py-2 rounded btn-primary text-white">Simpan</button>
				</div>
			</div>
		</form>
	</div>
