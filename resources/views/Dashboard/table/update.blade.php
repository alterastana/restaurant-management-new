...
<div class="bg-white rounded shadow p-6 max-w-lg mx-auto">
    <h2 class="text-lg font-bold mb-4">Edit Meja #{{ $table->table_number }}</h2>

    <form action="{{ route('Dashboard.table.update', $table->table_id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="restaurant_id" id="restaurant_id" class="w-full border rounded px-3 py-2">
                @foreach($restaurants as $restaurant)
                    <option value="{{ $restaurant->restaurant_id }}" {{ $restaurant->restaurant_id == $table->restaurant_id ? 'selected' : '' }}>
                        {{ $restaurant->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="number" name="table_number" id="table_number" value="{{ $table->table_number }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div>
            <label for="capacity" class="block font-medium">Kapasitas</label>
            <input type="number" name="capacity" id="capacity" value="{{ $table->capacity }}" class="w-full border rounded px-3 py-2" required>
        </div>
